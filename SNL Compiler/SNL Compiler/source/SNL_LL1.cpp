...
ProductionSet::ProductionSet(string prods_file_name) {
    ...
    makePordsFromFile(prods_file_name,m_productions);
...
bool ProductionSet::makePordsFromFile(const string& file_name,vector<Production>& to_set_prods) const
{
    vector<Production>& ret = to_set_prods;
    std::ifstream infile;
...
void ProductionSet::dfsBuildTree(Node*& parent,size_t& treeIndex,size_t& global_id,size_t& global_token_index)
{
    // 传过来的参数parent一定是非终极符
    ...
    for (unsigned int i = 0; i < parent->children.size(); i++) {
        if (is_Token_Terminal(parent->children[i]->curr)) {
            ...
            if (parent->children[i]->curr == TOKEN_BLANK) {}
                ...
                //cout << endl;
                parent->children[i]->curr_str = m_input_token_contains_stream[global_token_index];
                global_token_index++;
            }
            continue;// 终极符是叶节点
        }
        dfsBuildTree(parent->children[i],treeIndex,global_id,global_token_index);
    }
}


bool ProductionSet::grammarAnalysis()
{
    // 例 分析栈 #E   输入流 i+i*i#
    ...
    m_input_token_stream.emplace_back(TOKEN_SHARP);
    int i = 0;//输入流索引
    while (!s.empty()) {
        SNL_TOKEN_TYPE t = s.back();//栈顶元素
        SNL_TOKEN_TYPE r = m_input_token_stream[static_cast<unsigned int>(i)];//输入流当前扫描符号
...
void ProductionSet::setInputStrem(const vector<SNL_TOKEN_TYPE>& input_tkn_stream,
                                  const vector<string> input_tkn_contrain_stream)
{
    m_input_token_stream = input_tkn_stream;
    m_input_token_contains_stream = input_tkn_contrain_stream;
}
...
string ProductionSet::getTreeToStr() const{
    ...
    else return getTreeToStrSub("","",m_root);
}




string ProductionSet::getTreeToStrSub(string lftstr,string append,Node* node) const {
    string ap = append;
    if(node->curr == TOKEN_ID || node->curr == TOKEN_INTC || node->curr == TOKEN_CHARC){
        ap += get_Token_Str(node->curr) + "  :\t"+ node->curr_str;
    }else if(node->curr == TOKEN_BLANK){
         ap += "BLANK";
    }
    else if(is_Token_Terminal(node->curr)){
         ap += get_Token_Str(node->curr) ;
    }
    else{
        ap += get_Token_Str(node->curr);
    }
    ...
    if(children.size()>0){
        for(auto iter = children.begin();iter!=children.end()-1;iter++){
            ap+=(lftstr+getTreeToStrSub(lftstr+("  |"), ("  |-"),*iter));
        }
        ap +=(lftstr+getTreeToStrSub(lftstr+("   "), ("  |-"),*(children.end()-1)));
    }
